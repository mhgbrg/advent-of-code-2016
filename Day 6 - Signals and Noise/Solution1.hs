...
mostFrequentChar str = fst $ head sortedCounts
  where counts = charCounts str
        sortedCounts = sortBy compareCounts counts
        compareCounts (_, n1) (_, n2) = n2 `compare` n1

charCounts :: String -> [(Char, Int)]
charCounts str = map (\c -> (c, countOccurrences' c)) uniqueChars
  where uniqueChars = nub str
        countOccurrences' c = countOccurrences c str
